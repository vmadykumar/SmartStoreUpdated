...
#pragma warning restore 612, 618
        ...
        public static void AddRuleSets(IDbContext context)
        {
            ...
            void AddRulesForCommonFilters(SmartObjectContext soc, RuleSetEntity ruleSet, PluginFilterConfigModel model, string pluginSystemName)
            {
                // Store id (if plugin is limited to stores).
                var settingName = $"PluginSetting.{pluginSystemName}.LimitedToStores";
                var limitedToStoresValue = soc.Set<Setting>()
                ...
                if (model?.ExcludedCustomerRoles?.Any() ?? false)
                {
                    ...
                    if (excludedRoleIds.Any())
                    {
                        ...
                        else
                        {
                            ...
                            var allRoleIds = soc.Set<CustomerRole>().Select(x => x.Id).ToList();
